 ...
 */
class OrganisationLogoUploadService
{

    public function handle(OrganisationLogoRequest $request) {
    ...
    private function handleBase64Image(OrganisationLogoRequest $request) {
        $filenametostore = "b64_" . time();
        $imagePath = $this->saveImageFromBase64($request->image_base64, 'organisation_logos', $filenametostore);
        $thumbnailPath = $this->saveImageFromBase64($request->image_base64, 'organisation_logos/thumbnail', $filenametostore);
        $this->createThumbnail( public_path($thumbnailPath), 150, 150);

        return compact('imagePath', 'thumbnailPath', 'filenametostore');
    }

    private function handleFileImage(OrganisationLogoRequest $request) {
        ...
        $request->file('image')->storeAs('public/organisation_logos/thumbnail', $filenametostore);

        //create medium thumbnail
        $thumbnailPath = public_path('storage/organisation_logos/thumbnail/' . $filenametostore);
        $this->createThumbnail($thumbnailPath, 150, 150);

        $imagePath = 'storage/organisation_logos/' . $filenametostore;
        $thumbnailPath = 'storage/organisation_logos/thumbnail/' . $filenametostore;
