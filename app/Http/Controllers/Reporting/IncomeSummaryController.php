...
class IncomeSummaryController
{
    ...
    public function __invoke(IncomeSummaryRequest $request)
    {
        return Contribution::whereBetween('created', [$request->start_date, $request->end_date])->byCurrencyId($request->currency_id)
            ->with(['contributionType', 'contributionPaymentType'])
            ->selectRaw('module_contribution_type_id, module_contribution_payment_type_id, sum(amount) as amount')
            ->groupBy('module_contribution_type_id')->groupBy('module_contribution_payment_type_id')
            ->orderBy('module_contribution_type_id')->orderBy('module_contribution_payment_type_id')
